...
final class RTSPStreamManager: @unchecked Sendable {
    func test(camera: CameraConfig, password: String?, timeout: TimeInterval = 5) async -> ServiceTestResult {
        ...
        do {
            ...
            if firstLine.contains("401") {
                detail = "Camera responded to RTSP, but authentication was required or rejected. Digest auth cameras may still stream correctly through HomeKit/HAP."
            } else {
                detail = firstLine
            }
